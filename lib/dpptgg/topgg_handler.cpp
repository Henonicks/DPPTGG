...
dpptgg::sender_identification_statuses dpptgg::listener::identify_sender(dpp::http_server_request const* const request, nlohmann::json& payload_json) {
	...
	if (this->secrets.find(path) == this->secrets.end()) {
		this->log(dpp::ll_trace, "There are no secrets set for this endpoint.");
	...
	if (signature.find("t=") != 0) {
		this->log(dpp::ll_trace, "The signature doesn't start with a timestamp.");
		return sis_signature_no_timestamp;
	}
	if (request->get_header("content-type").find("application/json") != 0) {
		this->log(dpp::ll_trace, "The content type isn't json.");
	...
	for (; i < signature.size(); i++) {
		if (signature[i] >= '0' && signature[i] <= '9') {
			...
			if (signature[i] != ',') {
				this->log(dpp::ll_trace, "The top.gg signature timestamp contains non-digit characters.");
				return sis_signature_no_timestamp;
			}
			break;
		}
	...
	if (signature.size() - (timestamp.size() + 2) <= 4) {
		this->log(dpp::ll_trace, "There's not enough space for a top.gg signature.");
		return sis_bad_signature;
	}
	if (signature.substr(i + 1, 3) != "v1=") {
		this->log(dpp::ll_trace, "The v1 part of the signature is missing.");
		return sis_bad_signature;
	}
	...
	try {
		...
		this->log(dpp::ll_trace, "Couldn't parse the request body json.");
	...
	this->log(dpp::ll_trace, "None of the generated HMAC SHA256 digests match the V1 signature.");
...
std::string dpptgg::str_from_announcement_category(announcement_categories const category) {
	switch (category) {
	case ac_announcement:
		return "announcement";
	case ac_event:
		return "event";
	case ac_new_feature:
